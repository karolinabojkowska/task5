...
# set up pipeline scripts path
SCRIPT_PATH=$(cd $(dirname $0) && pwd)

#docker images
docker_pysradb=karolinabojkowska/docker-pysradb
VERSION_pysradb=v1
IMAGE_pysradb=$docker_pysradb:$VERSION_pysradb

docker_plotmetadata=karolinabojkowska/docker-plotmetadata
VERSION_plotmetadata=v1
IMAGE_plotmetadata=$docker_plotmetadata:$VERSION_plotmetadata
...
# pull images and evaluate

#checkdockerimagepull $docker_pysradb $log_file 

if [ ! -n  "checkdockerimage $docker_pysradb" ] ; then
	docker pull $IMAGE_pysradb >> $log_file 2>&1
	checkdockerimagepull $docker_pysradb $log_file 
else
	infomsg $log_file "Docker image $IMAGE_pysradb already present"
fi

if [ ! -n  "checkdockerimage $docker_plotmetadata" ] ; then
        docker pull $IMAGE_plotmetadata >> $log_file 2>&1
        checkdockerimagepull $docker_plotmetadata $log_file 
else
        infomsg $log_file "Docker image $IMAGE_plotmetadata already present"
fi

#docker pull $IMAGE_plotmetadata >& $log_file
